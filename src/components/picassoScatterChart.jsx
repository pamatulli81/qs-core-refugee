...
export default class PicScatterplot extends React.Component {
  static hideTooltip = () => {
    ...
    if (!pic) {
      ...
      picassoPic.brush("tooltip").on("update", added => {
        if (added.length) {
          ...
          PicScatterplot.showTooltip(s.data.country.value, p);
        } else {
          PicScatterplot.hideTooltip();
        }
      });
